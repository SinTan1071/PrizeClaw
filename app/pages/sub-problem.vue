<template>
    <div class="my-prize">
        ...
        <div  class="problem-check">
            <p style="margin-top: 10px;display: inline-block">选择退换原因</p>
            <div style="width:55px ;height:15px;border: 1px solid #F63A52; border-radius: 4px;display: inline-block;float: right;margin-right: 12px;padding:5px 0 10px 10px ;margin-top:6px;color:#F63A52">发错货</div>
        </div>
        <div class="logistics">
            <x-input title="填写退货物流单号:"  placeholder="请输入退货物流单号"  placeholder-align="right" v-model="num"></x-input>
        </div>
        <div class="change-good">
            <p >填写退换说明:<span style="color: #3D3D3D;margin-left: 5px">(选填)</span></p>
            <x-textarea :max="200" name="detail" placeholder="填写退换说明" :show-counter="false" style="padding: 0;font-size: 16px;line-height: 21px;" v-model="tips"></x-textarea>
        </div>
        <div class="upload ">
            <p >上传凭证</p>
            <div class="pic"  >
        ...
        <x-button class="submit-problem" action-type="button"  link="/home">返回游戏</x-button>
    </div>
</template>
<script>
    ...
    let $data = {
        isAgree:false,
        num:'12345678919011111',
        tips:'我是退换说明，我有点长，您慢慢看！我是退换说明，我有点长，\n' +
        '您慢慢看！',
        problemCheck:'发错货',
        problemItem:['发错货'],
    };
